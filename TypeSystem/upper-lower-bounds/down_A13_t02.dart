...
///   - `Never` otherwise
///
/// @description Check that DOWN(`T1`, `T2`) = NonNull(`T1`) if `T1 != T2`, `T1`
/// and `T2` are neither TOP, BOTTOM, NULL, if OBJECT(`T2`), not OBJECT(`T1`),
/// `T1` is not non-nullable and NonNull(`T1`) is non-nullable.
...
void f12a(void Function(Never?) v1, void Function(Object) v2) {
  // DOWN(Never?, Object) = Never because NonNull(Never?) = Never
  var v = (1 > 2) ? v1 : v2;
  v.expectStaticType<Exactly<void Function(Never)>>();
}

void f12b(void Function(Never?) v1, void Function(FutureOr<Object>) v2) {
  var v = (1 > 2) ? v1 : v2;
  v.expectStaticType<Exactly<void Function(Never)>>();
}
